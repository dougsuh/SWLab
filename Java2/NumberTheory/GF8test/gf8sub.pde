// Java does not support unsigned byte
public class GF8 {
  private static final int FIELD_SIZE = 8;
  private static final int PRIMITIVE_POLYNOMIAL = 0xb;  // 1011
  private final int[] expTable = new int[FIELD_SIZE * 2];
  private final int[] logTable = new int[FIELD_SIZE];

  public GF8() {
    initializeTables();
    System.out.println("GF8 \nexp log");
    for (int i=0; i<FIELD_SIZE; i++) {
      System.out.println("  "+expTable[i] + " " + logTable[i]);
    }
  }
  private void initializeTables() {
    int x = 1;
    for (int i = 0; i < FIELD_SIZE; i++) {
      expTable[i] = x;
      logTable[x] = i;

      x <<= 1; // multiply by 2
      if (x >= FIELD_SIZE) {
        x ^= PRIMITIVE_POLYNOMIAL;
      }
    }
    logTable[0] = 0;   // not defined, but, for convenience
    // Copy expTable[0..255] to expTable[256..511] for overflow handling
    for (int i = FIELD_SIZE; i < expTable.length; i++) {
      expTable[i] = expTable[i - FIELD_SIZE];
    }
  }
  public int add(int a, int b) {
    return a ^ b; // addition in GF(256) is XOR
  }
  public int subtract(int a, int b) {
    return a ^ b; // same as addition
  }
  public int multiply(int a, int b) {
    if (a == 0 || b == 0) return 0;
    int logSum = logTable[a] + logTable[b];
    if(logSum>7) logSum -= 7; // suh 2026.1.20
    return expTable[logSum];
  }
  public int inverse(int a) {
    if (a == 0) throw new ArithmeticException("Cannot invert 0 in GF(256)");
    return expTable[7 - logTable[a]];
  }
  public int divide(int a, int b) {
    if (b == 0) throw new ArithmeticException("Division by zero in GF(256)");
    if (a == 0) return 0;
    int logDiff = logTable[a] - logTable[b]; // suh
    if (logDiff<0) logDiff += 7;
    return expTable[logDiff];
  }
  // Optional: for displaying as hex
  public String toHex(int value) {
    return String.format("0x%02X", value);
  }
  // ax + by = c         dx + ey = f      
  int [] Eq2(int a, int b, int c, int d, int e, int f) {
    int det = add(multiply(a, e), multiply(b, d));
    int x,y;
    if (det == 0) {
      throw new ArithmeticException("Division by zero in GF(8)");
    } else {
      x = add(multiply(b, f), multiply(c, e));
      x = divide(x, det);
      y = add(multiply(a, f), multiply(c, d));
      y = divide(y, det);
    }
    int [] xy = new int[2]; xy[0] = x; xy[1] = y; 
    return xy;
  }
}
